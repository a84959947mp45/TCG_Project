...
public:
	...
	inline piece_type AgainstColor(piece_type c) {
		if(c == piece_type::black)  return piece_type::white;
		else if(c == piece_type::white)  return piece_type::black;
		return piece_type::unknown;
	}
...
public:
	...
	struct point {
		int x, y, i;
		point(int i = -1, board::piece_type color = board::piece_type::empty) : x(i != -1 ? i / size_y : -1), y(i != -1 ? i % size_y : -1), i(i), color(color) {}
		point(int x, int y, board::piece_type color = board::piece_type::empty) : x(x), y(y), i(x != -1 && y != -1 ? x * size_y + y : -1), color(color) {}
...
public:
	...
	reward place(const point& p, piece_type who = piece_type::unknown) {
        
		reward result = place(p.x, p.y, who);
		if(result == nogo_move_result::legal)
		{
			m_vMoves.push_back(point(p.x, p.y, who));
			setTurnColor(AgainstColor(getTurnColor()));
		}
		return result;
...
public:
    ...
    inline void setTurnColor(board::piece_type c) { m_turnColor = c; }

	vector<point> getMoves() { return m_vMoves; }
	inline board::piece_type getTurnColor() const { return m_turnColor; }

	void undo() {
		stone[m_vMoves.back().x][m_vMoves.back().y] = piece_type::empty;
		m_turnColor = m_vMoves.back().color;
		attr.who_take_turns = m_vMoves.back().color;
		m_vMoves.pop_back();
	}

	board::piece_type eval()
	{ 
		if(isTerminal())
		{
			return AgainstColor(getTurnColor());
		}

		return board::piece_type::empty;
	}
	...
	bool isTerminal()
	{
		...
		for(int x = 0; x < size_x; x++){
			for(int y = 0; y < size_y; y++){
				...
				if(!isLegalMove(point(x,y, getTurnColor()))) continue;
				legalMove.push_back(point(x, y,  getTurnColor()));
			}
		}

		if(legalMove.empty()) return true;
		return false;
	}

	bool isLegalMove(point position)
	{
        int x = position.x;
		int y = position.y;
		board::piece_type who = position.color;
		point p_min(0, 0), p_max(size_x - 1, size_y - 1);
		if (x < p_min.x || x > p_max.x || y < p_min.y || y > p_max.y) return false;
		if (board::initial()[x][y] == piece_type::hollow)             return false;
		board test = *this;
		if (test[x][y] != piece_type::empty) return false;
		test[x][y] = who; // try put a piece first
		if (test.check_liberty(x, y, who) == 0) return false;
		unsigned opp = 3u - who;
		if (x > p_min.x && test.check_liberty(x - 1, y, opp) == 0) return false;
		if (x < p_max.x && test.check_liberty(x + 1, y, opp) == 0) return false;
		if (y > p_min.y && test.check_liberty(x, y - 1, opp) == 0) return false;
		if (y < p_max.y && test.check_liberty(x, y + 1, opp) == 0) return false;
		return true;
	}
...
private:
	grid stone;
	data attr;
	vector<point> m_vMoves;
	board::piece_type m_turnColor;
};
